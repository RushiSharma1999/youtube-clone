...
function Sidebar() {
    ...
    const otherLinks = [
        ...
        {icon: <MdOutlineWatchLater className={'text-xl'}/>, name: "WatchLater"},
        {icon: <MdOutlineThumbUp className={'text-xl'}/>, name: "Liked Videos"},
    ];
    return (
        <div className={'w-2/12 bg-[#212121] p-2 pr-5 overflow-auto pb-8 h-screen'}>
            <ul className={'flex flex-col border-b-2 border-gray-700'}>
                {mainLinks.map(({icon, name}) => {
                    return (
                        <li key={name}
                            className={`pl-6 py-3 hover:bg-zinc-600 ${name === "Home" ? "bg-zinc-600" : " "} rounded-xl`}>
